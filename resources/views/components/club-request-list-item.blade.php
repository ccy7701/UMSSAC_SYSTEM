...
<div class="rsans card club-request-list-item h-100 px-0" id="club-request-list-item-{{ $request->creation_request_id }}">
    <div class="row g-0 align-items-center" data-bs-toggle="collapse" data-bs-target="#actions-{{ $request->creation_request_id }}" aria-expanded="false" aria-controls="actions-{{ $request->creation_request_id }}">
        @php
            $clubImagePaths = json_decode($request->club_image_paths, true);
        @endphp
        <!-- Image section-->
        <div class="col-xl-2 col-lg-2 col-md-2 col-sm-3 col-xs-3 col-3">
        ...
        <!-- Content section -->
        <div class="col-xl-10 col-lg-10 col-md-10 col-sm-9 col-xs-9 col-9">
        ...
        @if ($request->request_status == 0)
            ...
            <div id="actions-{{ $request->creation_request_id }}" class="collapse">
                ...
                <div class="container px-2 py-xl-2 py-lg-2 py-md-2 py-0">
                    <div class="row">
                        <div class="club-request-actions-row d-flex justify-content-center col-12 mb-3 px-0">
                            <a href="{{ '#' }}" class="section-button-extrashort rsans btn btn-secondary fw-semibold px-3 me-3">Review request</a>
                            <button type="button" class="section-button-extrashort rsans btn btn-danger fw-semibold px-3"
                                data-bs-toggle="modal"
                                data-bs-target="#reject-confirmation-modal"
                                data-creation-request-id="{{ $request->creation_request_id }}"
                                data-club-name="{{ $request->club_name }}">
                                Reject request
                            </button>
                        </div>
                    </div>
